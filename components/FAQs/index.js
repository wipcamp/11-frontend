...
export default class Question extends Component {
  ...
  render() {
    const rotateDeg = this.state.rotate ? '0deg' : '180deg';
    return (
      <Bg1>
        <Background />
                      ...
                      <div id="accordion" className={rotateDeg}>
                        {
                          QandA.map((data, i) => {
                            return (
                              <React.Fragment>
                                <FontFAQs>
                                <QuestionText className="card-link bg-secondary" data-toggle="collapse" href={`#collapse${this.state.num[i]}`} key={i}>
                                  <div className="card-header px-4" onClick={({ target: { i } }) => this.handleOnClick(i)}>
                                    Q : {data.question} ?
                                    <Arrow
                                      className={i}
                                      id={i}
                                      ...
                                      deg={rotateDeg}
                                      index={i}
                                      onClick={({ target: { i } }) => this.handleOnClick(i)}
                                      />
                                  </div>
                                </QuestionText>
                                
                                <div id={`collapse${this.state.num[i]}`} className = {`collapse`}  data-parent="#accordion">
